...
import { PlutoVoiceSessionRuntime, type PlutoVoiceSessionRegistration } from "./pluto-voice-session-runtime.js";
...
export class PlutoService {
	...
	constructor(
		...
		_unusedExecuteTool: unknown = null,
		private readonly isVoiceAudioEnabled: () => boolean = () => true,
	) {
	...
	getModel() {
		return this.env.PLUTO_CODEX_MODEL;
	}
	...
	async executeVoiceSessionTool(
		...
		const runtime = this.getOrCreateVoiceSessionRuntime(sessionId);
		return runtime.executeLocalTool(toolName, payload, options);
	}
