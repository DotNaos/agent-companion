...
export function App() {
  ...
  useEffect(() => {
    if (
      ...
      !plutoVoiceSessions.some((session) => session.id === activeVoiceSessionId)
    ) {
          ...
          <div className="grid gap-4 md:grid-cols-3 auto-rows-min">
            ...
            <Card className="md:col-span-1">
              ...
              <CardContent className="flex items-center justify-between">
                ...
                <Button
                  variant={
                    bootstrap?.desktop.tunnelRunning ? "outline" : "default"
                  }
                  size="sm"
            ...
            >
              <CardHeader>
                ...
                <CardDescription>Review pending tool requests</CardDescription>
              </CardHeader>
              <CardContent className="flex-1 flex flex-col justify-end">
            ...
            <Card className="md:col-span-3 bg-white/5 border-white/10">
              ...
              <CardContent className="grid gap-8 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1.8fr)] xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                {/* Left Side: General Pluto Settings */}
                <div className="flex flex-col gap-4">
                  <h3 className="text-sm font-medium text-slate-200 px-1">
                    Settings
                  </h3>
                  <div className="flex flex-col rounded-2xl border border-white/10 bg-black/20 overflow-hidden text-sm">
                    <div className="flex items-center justify-between p-4 border-b border-white/5">
                      ...
                      <Button
                        ...
                        onClick={() => void triggerPlutoCommentary()}
                        disabled={
                          plutoTriggerPending ||
                          !bootstrap?.runner.pluto.available ||
                    ...
                    <div className="flex items-center justify-between p-4 border-b border-white/5">
                      ...
                      <Switch
                        checked={
                          draftConfig?.pluto.autoCommentaryEnabled ?? false
                        }
                ...
                <div className="flex flex-col gap-4">
                  <div className="flex items-center justify-between px-1">
                    ...
                    <Button
                      size="sm"
                      onClick={() => void createPlutoVoiceSession()}
                      disabled={plutoSessionActionPending === "create"}
                      className="h-8 px-4 font-medium text-xs bg-emerald-500 text-slate-950 hover:bg-emerald-400 transition-colors"
                    >
                      ...
                      plutoVoiceSessions.map((session) => {
                        ...
                        return (
                          <div
                            key={session.id}
                            className={cn(
                              "group flex flex-col gap-3 rounded-2xl border p-4 transition-colors relative overflow-hidden",
                              isSelectedSession
                              ...
                              <div className="space-y-1.5 flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                  ...
                                  {session.speakerClientId && (
                                    <Badge
                                      variant="secondary"
                                      className="shrink-0 bg-black/40 text-[10px] uppercase font-semibold text-emerald-400 border-emerald-400/20 px-2 py-0 h-4 rounded-full hidden sm:inline-flex"
                                    >
                                      Mic active
                                    </Badge>
                                  )}
                                ...
                                <div className="text-xs text-slate-400 mt-1 line-clamp-2">
                                  {isCurrentSpeaker
                                    ...
                                    : isSpeakerOccupiedByOtherClient
                                      ...
                                      : knownClientId
                                        ? "You are observing."
                                        : "Mic is open. Join as speaker to talk."}
                                </div>
                              </div>
                              ...
                              <div className="flex flex-col items-end gap-2 shrink-0">
                                <div className="flex items-center gap-2">
                                  <Button
                                    ...
                                    onClick={() =>
                                      knownClientId
                                        ? void joinPlutoVoiceSession(session)
                                        : void joinPlutoVoiceSessionAs(
                                            session,
                                            "observer",
                                          )
                                    }
                                    disabled={sessionBusyKey}
                                  >
                                    {observerButtonLabel}
                                  </Button>
                                  ...
                                  {!isSpeakerOccupiedByOtherClient &&
                                    !isCurrentSpeaker && (
                                      <Button
                                        variant="default"
                                        size="sm"
                                        className="h-8 text-xs bg-white text-black hover:bg-slate-200 hidden sm:inline-flex"
                                        onClick={() =>
                                          void joinPlutoVoiceSessionAs(
                                            session,
                                            "speaker",
                                  ...
                                  <Button
                                    ...
                                    onClick={() =>
                                      void closePlutoVoiceSession(session.id)
                                    }
                                    disabled={sessionBusyKey}
                                    title="Close session"
                                  >
                  ...
                  {activeVoiceSessionId ? (
                    <div className="mt-2">
                      <div className="mb-3 rounded-2xl border border-dashed border-emerald-400/20 bg-emerald-400/5 p-3 text-xs text-slate-300">
                        Controls mirrored from the desktop avatar — configure
                        audio devices here.
                      </div>
              ...
              <>
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Base directory for your workspace and development
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <Input
                      ...
                      onChange={(event) =>
                        updateDraftConfig((current) => ({
                          ...current,
                          projectsRoot: event.target.value || null,
                        }))
                      }
                  ...
                  <CardHeader>
                    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                      <div>
                        ...
                        <CardDescription>
                          Add directories that the MCP may access. The mode
                          below applies globally to every path in this list.
                        </CardDescription>
                      </div>
                      <Button
                        variant="secondary"
                    ...
                    <div className="grid gap-2 md:max-w-sm">
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          updateAccessMode(event.target.value as MCPAccessMode)
                        }
                      ...
                      <p className="text-xs text-slate-500">
                        {
                          ACCESS_MODE_OPTIONS.find(
                            (option) =>
                              option.value === draftConfig.mcpAccessMode,
                          )?.description
                        }
                      </p>
                    </div>
                      ...
                      draftConfig.allowedPaths.map((entry) => (
                        ...
                        >
                          <Input
                            value={entry.path}
                            onChange={(event) =>
                              updatePathValue(entry.id, event.target.value)
                            }
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => void browseForPath(entry.id)}
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => removePathEntry(entry.id)}
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-3">
                      ...
                      <Button
                        variant="ghost"
                        onClick={() =>
                          decideApproval(approval.id, "denied", false)
                        }
        ...
        {currentView === "activity" && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Auth, tool calls, writes, command execution, and process
                lifecycle events.
              </CardDescription>
            </CardHeader>
            <CardContent>
  ...
  async function toggleTunnel() {
    try {
      ...
      syncBootstrap(result);
      setStatusMessage(
        result.desktop.tunnelRunning ? "Tunnel started." : "Tunnel stopped.",
      );
      setTimeout(() => setStatusMessage(null), 2000);
    } catch (requestError) {
      setError(
    ...
    try {
      ...
      const result = await apiRequest<PlutoVoiceSessionCreateOutput>(
        ...
        "POST",
        {
          ...
          client: {
            label: mode === "admin" ? "Remote Admin" : "Desktop Companion",
  ...
  async function joinPlutoVoiceSessionAs(
    ...
    try {
      ...
      if (existingClientId) {
        await apiRequest(`/pluto/sessions/${session.id}/detach`, "POST", {
          ...
          <div className="pet-bubble">
            ...
            <div className="pet-bubble-actions">
              ...
              <button
                ...
                onClick={() => void decideFromOverlay("approved", true)}
              >
                Approve + Remember
              </button>
              <button
                className="pet-bubble-button primary"
                onClick={() => void decideFromOverlay("approved", false)}
              >
                Approve Once
              </button>
            </div>
        ...
        {items.map((task) => (
          <div key={task.id} className="flex items-center gap-3">
            ...
            <Button
              ...
              size="icon"
              onClick={() =>
                onChange(items.filter((entry) => entry.id !== task.id))
              }
