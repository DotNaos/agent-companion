            ...
            <Card>
              <CardHeader>
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Add directories that the MCP may access. The mode below
                      applies globally to every path in this list.
                    </CardDescription>
                  </div>
                  <Button variant="secondary" onClick={onAddPathEntry} size="sm">
                    <Plus className="mr-1.5 h-4 w-4" /> Add Path
                  </Button>
                </div>
                <div className="grid gap-2 md:max-w-sm">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      onUpdateAccessMode(event.target.value as MCPAccessMode)
                    }
                  ...
                  <p className="text-xs text-slate-500">
                    {
                      ACCESS_MODE_OPTIONS.find(
                        (option) => option.value === draftConfig.mcpAccessMode,
                      )?.description
                    }
                  </p>
                </div>
              ...
              <CardContent className="grid gap-4">
                {draftConfig.allowedPaths.length === 0 ? (
                  <p className="text-sm text-slate-400">No paths added yet.</p>
                ) : (
                  draftConfig.allowedPaths.map((entry) => (
                    ...
                    >
                      <Input
                        value={entry.path}
                        onChange={(event) =>
                          onUpdatePathValue(entry.id, event.target.value)
                        }
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => void onBrowseForPath(entry.id)}
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => onRemovePathEntry(entry.id)}
