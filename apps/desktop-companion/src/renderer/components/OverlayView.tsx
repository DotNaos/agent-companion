...
function bindAudioAnalyser(
  ...
  const connectAnalyser = () => {
    try {
      ...
      if (!(audio as HTMLAudioElement & { _hasSourceConnected?: boolean })._hasSourceConnected) {
        const source = ctx.createMediaElementSource(audio);
        source.connect(analyser);
        analyser.connect(ctx.destination);
        (audio as HTMLAudioElement & { _hasSourceConnected?: boolean })._hasSourceConnected = true;
      }
