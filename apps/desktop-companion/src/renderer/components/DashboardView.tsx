...
export function DashboardView({
    ...
    const plutoAvailable = bootstrap?.runner.pluto.available ?? false;
    const plutoLastError = bootstrap?.runner.pluto.lastError ?? null;
                ...
                <CardContent className="grid gap-8 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1.8fr)] xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-black/20 text-sm">
                            ...
                            {plutoAvailable ? null : (
                                <div className="border-b border-rose-400/10 bg-rose-500/10 p-4 text-xs text-rose-100">
                                    {plutoLastError ||
                                        'Gemini is not configured for live Pluto voice yet. Add GEMINI_API_KEY to your secrets env and restart the local runner.'}
                                </div>
                            )}
                            ...
                            {plutoVoiceSessions.length === 0 ? (
                                ...
                                plutoVoiceSessions.map((session) => (
                                    <VoiceSessionCard
                                        ...
                                        session={session}
                                        knownClientId={
                                            localSessionClients[session.id] ??
                                            null
                                        }
                                        isSelectedSession={
                        ...
                        {activeVoiceSessionId ? (
                            <div className="mt-2">
                                <div className="mb-3 rounded-2xl border border-dashed border-emerald-400/20 bg-emerald-400/5 p-3 text-xs text-slate-300">
                                    Controls mirrored from the desktop avatar —
                                    configure audio devices here.
                                </div>
                                <PlutoVoiceSessionConsole
                                    ...
                                    variant="panel"
                                    onRequestSpeaker={(sessionId) => {
                                        const session = plutoVoiceSessions.find(
                                        ...
                                        return onJoinPlutoVoiceSessionAs(
                                            session,
                                            'speaker',
                                        );
                                    }}
...
function VoiceSessionCard({
    ...
    const takeMicButton = getTakeMicButtonState({
        isBusy,
        isCurrentSpeaker,
            ...
            <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex shrink-0 flex-col items-end gap-2">
                    <div className="flex flex-wrap items-center justify-end gap-2">
                        <Button
                            variant={
                                isSelectedSession ? 'outline' : 'secondary'
                            }
                            size="sm"
                            className={cn(
                                'h-8 text-xs',
                        ...
                        {takeMicButton.visible ? (
                            <Button
                                variant="default"
                                size="sm"
                                className="h-8 bg-white text-xs text-black hover:bg-slate-200"
                                onClick={() =>
                                    void onJoinSessionAs(session, 'speaker')
                                }
                                disabled={takeMicButton.disabled}
                                title={takeMicButton.title}>
                                Take mic
                            </Button>
                        ) : null}
                        <Button
...
export function getTakeMicButtonState({
    isBusy,
    isCurrentSpeaker,
    isSpeakerOccupiedByOtherClient,
    plutoAvailable,
    ...
    return {
        disabled: isBusy || !plutoAvailable || isSpeakerOccupiedByOtherClient,
        title: isSpeakerOccupiedByOtherClient
            ? 'Available when the current speaker releases the mic.'
            : undefined,
        visible: true,
    };
}
